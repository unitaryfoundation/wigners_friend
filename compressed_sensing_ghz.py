...
from ewfs.ghz import GHZ, FidelityEstimator, post_select_results
...
class CompressedSensingEstimator:
    ...
    def fit(self, phases, signal_values):
        ...
        D = np.hstack([
            np.cos(np.outer(phases, self.n_candidates)), 
            -np.sin(np.outer(phases, self.n_candidates))
        ])
        ...
        D_refine = np.column_stack([
            np.cos(recovered_n * phases),
            -np.sin(recovered_n * phases)
        ])

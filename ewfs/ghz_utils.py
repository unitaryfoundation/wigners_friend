...
def remove_duplicate_coupling(coupling_map: list[tuple[int]]) -> list[tuple[int]]:
    """
    ...
    for qubit_0, qubit_1 in coupling_map:
        if ((qubit_0, qubit_1) in new_coupling_map) or (
            (qubit_1, qubit_0) in new_coupling_map
        ):
            continue
        else:
...
def get_physical_to_virtual_map(
    physical_qubits: list[int], num_qubits: int
) -> dict[int, int]:
    """
...
def construct_ghz(
    ...
    """
    coupling = sorted(list(backend.coupling_map))
    if qubits_to_remove:
        coupling = remove_qubits_from_coupling_map(coupling, qubits_to_remove)
    coupling = nx.from_edgelist(coupling)

    _, visited_edges, _, visited_nodes_list = expand_graph(
        coupling, start_qubit, max_nodes_per_step=1
    )
    physical_to_virtual_map = get_physical_to_virtual_map(
...
def single_step(
    ...
    for node in list(visited_nodes):
        ...
        unvisited_neighbors = [
            neighbor for neighbor in neighbors if neighbor not in visited_nodes
        ]
...
class GHZCircuitBuilder:
    ...
    def __init__(
        ...
        self.coupling = sorted(remove_duplicate_coupling(list(backend.coupling_map)))
        self.flags_physical = flags_physical
        self.num_flag_qubits = len(flags_physical)
        self.flags_physical2virtual = {
            x: self.num_ghz_qubits + flags_physical.index(x) for x in flags_physical
        }
    ...
    def build(self) -> dict[str, Union[list[int], QuantumCircuit]]:
        ...
        circuit_without_flags, layout_without_flags, physical_to_virtual_map = (
            construct_ghz(
        ...
        for a, b in self.coupling:
            if ((a in self.flags_physical) or (b in self.flags_physical)) and (
                ...
                flag_cxs_virtual.append(
                    [physical_to_virtual_map[ctrl], self.flags_physical2virtual[targ]]
                )
                assert targ in self.flags_physical

        # build the circuit with parity checks
        qregs = QuantumRegister(
            circuit_without_flags.num_qubits + len(self.flags_physical), "q"
        )
        circuit_with_flags = QuantumCircuit(qregs)
        circuit_with_flags.compose(
            circuit_without_flags, qubits=qregs[: self.num_ghz_qubits], inplace=True
        )
...
def plot_ghz_fez(
    ...
    start_qubit: int = None,
    swap_flag_qubits: list[int] = None,
    extra=None,
    fig_size=(7, 7),
):
